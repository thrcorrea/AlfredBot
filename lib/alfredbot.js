...
AlfredBot.prototype._MostraFosSprint = function(originalMessage){
    ...
    pg.connect(database.url, function(err, client, done){
       ...
       var query = client.query("select numero_fo from gerente.fos where cod_funcionario_responsavel_fo = (select id_funcionario from gerente.funcionarios where nm_funcionario ilike 'thales')");
