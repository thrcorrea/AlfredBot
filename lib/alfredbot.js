...
AlfredBot.prototype._carregaUsuarios = function(){
    for(var index = 0; index < objetoBot.users.length; index++){
        objetoUsuario = new Usuario();
         objetoUsuario.setId(objetoBot.users[index].id);
         objetoUsuario.setNome(objetoBot.users[index].name);
...
AlfredBot.prototype._respondeUsuario = function (message){
    for (var index = 0; index < usuarios.length; index++){
        if (message.user == usuarios[index].getId()){           
            this.postMessageToUser(usuarios[index].getNome(), 'Olá mestre ' + usuarios[index].getNome(), {as_user: true});
        }
    } 
};
